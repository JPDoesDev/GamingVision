...
/// <summary>
/// Service for handling global hotkeys for crosshair toggle.
/// Uses a message-only window for hotkey registration.
/// </summary>
public class CrosshairHotkeyService : IDisposable
{
    private const int HOTKEY_ID = 9001; // Different from overlay (9000)
    ...
    public CrosshairHotkeyService(string hotkeyString)
    {
    ...
    public void Stop()
    {
        ...
        try
        {
            ...
            {
                Logger.Log("CrosshairHotkeyService: Thread did not join in time");
            }
            _messageThread = null;
        ...
        {
            Logger.Error($"CrosshairHotkeyService.Stop() error: {ex.Message}", ex);
        }
    }
    ...
    private void MessageLoop()
    {
        ...
        var wndClass = new WNDCLASSEX
        {
            ...
            lpszClassName = $"GamingVisionCrosshairHotkeyClass_{Environment.TickCount}"
        };
        ...
        if (_hwnd == IntPtr.Zero)
        {
            Logger.Warn("CrosshairHotkeyService: Failed to create hotkey window");
            return;
        }
        ...
        if (ParseHotkey(_hotkeyString, out uint modifiers, out uint vk))
        {
            ...
            {
                Logger.Warn($"CrosshairHotkeyService: Failed to register hotkey: {_hotkeyString}");
            }
            else
            {
                Logger.Log($"CrosshairHotkeyService: Registered hotkey: {_hotkeyString}");
            }
        }
